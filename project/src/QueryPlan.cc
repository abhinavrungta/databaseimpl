...
void QueryPlan::updateInput(struct FuncOperator *finalFunction,
...
void QueryPlan::clearQuery() {
	pipeNum = 0;
	root = NULL;
	QueryPlanNode::relOpList.clear();
	QueryPlanNode::pipesList.clear();
}

void QueryPlan::GetJoinsAndSelects(vector<AndList*> &joins,
...
map<string, AndList*>* QueryPlan::OptimizeSelectAndApply(
...
vector<AndList*>* QueryPlan::OptimizeJoinOrder(vector<AndList*> joins) {
...
OrderMaker *QueryPlan::GenerateOM(Schema *schema) {
...
Function *QueryPlan::GenerateFunc(Schema *schema) {
	Function *func = new Function();
	func->GrowFromParseTree(this->finalFunction, *schema);
	return func;
}

int QueryPlan::CreatePlan() {
	...
	for (TableList *table = this->tables; table != NULL; table = table->next) {
		...
		selectFile->outPipeId = this->pipeNum++;
	...
	if (selAboveJoin.size() > 0) {
		...
		selAbvJoin->outPipeId = this->pipeNum++;
	...
	if (this->groupAtts != NULL) {
		...
		groupBy->outPipeId = this->pipeNum++;
		groupBy->CreatePipe();
	...
	project->outPipeId = this->pipeNum++;
	...
	if (this->distinctAtts) { //we have distinct
		...
		distinct->outPipeId = this->pipeNum++;
		distinct->CreatePipe();
		this->root = distinct;
	}
	return 1;
