...
Type Function::RecursivelyBuild(struct FuncOperator *parseTree,
	...
	if (parseTree->right == 0 && parseTree->leftOperand == 0
		...
		if (parseTree->leftOperand->code == NAME) {
			...
			if (myNum == -1) {
				cerr
						<< "Error!  Attribute in arithmatic expression was not found.\n";
				exit(1);
			}
			...
			*((int *) opList[numOps].litInput) = atoi(
					parseTree->leftOperand->value);
			numOps++;
			return Int;
			...
			*((double *) opList[numOps].litInput) = atof(
					parseTree->leftOperand->value);
			numOps++;
			return Double;
		}
...
Type Function::Apply(Record &toMe, int &intResult, double &doubleResult) {
	...
	if (lastPos != stack) {
		...
		cerr
				<< "During function evaluation, we did not have exactly one value ";
