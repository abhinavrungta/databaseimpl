...
double Statistics::Estimate(struct AndList *parseTree, char **relNames,
	...
	for (int i = 0; i < numToJoin; i++) {
		relIter = this->relationData->find(*relNames);
...
//operands consists of a code and value.
	while (currentAnd != NULL) {
		...
		while (currentOr != NULL) {
			...
			if (isJoin) {
				...
				if (currentCompOp->code == GREATER_THAN
	...
	cout << resultEstimate << endl;

	if (isApply) {
		map<string, int>::iterator relOpMapITR, distinctCountMapITR;
		set<string> addedJoinAttrSet;
		if (isJoinPerformed) {
			for (relOpMapITR = relOpMap.begin(); relOpMapITR != relOpMap.end();
					relOpMapITR++) {

				for (int i = 0; i < relationData->size(); i++) {
					if (relNames[i] == NULL)
						continue;
					int cnt = ((*attrData)[relNames[i]]).count(
							relOpMapITR->first);
					if (cnt == 0)
						continue;
					else if (cnt == 1) {

						for (distinctCountMapITR =
								(*attrData)[relNames[i]].begin();
								distinctCountMapITR
										!= (*attrData)[relNames[i]].end();
								distinctCountMapITR++) {
							if ((relOpMapITR->second == LESS_THAN)
									|| (relOpMapITR->second == GREATER_THAN)) {
								(*attrData)[joinLeftRelation + "_"
										+ joinRightRelation][distinctCountMapITR->first] =
										(int) round(
												(double) (distinctCountMapITR->second)
														/ 3.0);
							} else if (relOpMapITR->second == EQUALS) {
								if (relOpMapITR->first
										== distinctCountMapITR->first) { //same attribute on which condition is imposed
									(*attrData)[joinLeftRelation + "_"
											+ joinRightRelation][distinctCountMapITR->first] =
											1;
								} else
			...
			if (addedJoinAttrSet.count(joinLeftRelation) == 0) {
				for (map<string, int>::iterator entry =
						(*attrData)[joinLeftRelation].begin();
						entry != (*attrData)[joinLeftRelation].end(); entry++) {
					(*attrData)[joinLeftRelation + "_" + joinRightRelation][entry->first] =
							entry->second;
			...
			if (addedJoinAttrSet.count(joinRightRelation) == 0) {
				for (map<string, int>::iterator entry =
						(*attrData)[joinRightRelation].begin();
						entry != (*attrData)[joinRightRelation].end();
						entry++) {
					(*attrData)[joinLeftRelation + "_" + joinRightRelation][entry->first] =
							entry->second;
			...
			(*relationData)[joinLeftRelation + "_" + joinRightRelation] = round(
					resultEstimate);
